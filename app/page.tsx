...
export default function HomePage() {
  return <main>
    <header className="masthead"><Link className="brand" href="/"><span>FADS</span> Road Response</Link><nav className="header-links"><a href="/voice">Voice report</a><Link href="/dispatch">Dispatcher board</Link></nav></header>
    ...
    <section className="report-shell"><div className="section-heading"><p className="eyebrow">New road report</p><h2>Where should we send the crew?</h2><div className="report-modes" aria-label="Report mode"><span className="active-mode" aria-current="page">Photo report</span><a href="/voice">Voice report</a></div></div><ReportForm workflows={Object.values(WORKFLOWS).map(({id,label,description})=>({id,label,description}))}/></section>
