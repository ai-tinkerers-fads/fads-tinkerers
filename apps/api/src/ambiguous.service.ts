...
type FadsResources = {
  projectId?: string;
  statusIds?: { reported?: string };
};

function fadsResources(): FadsResources {
  const path = process.env.FADS_RESOURCES_PATH;
  if (!path) return {};
  try {
    return JSON.parse(readFileSync(path, "utf8")) as FadsResources;
  } catch {
    return {};
  }
}
...
export class AmbiguousService {
  async createTask(
    ...
    const token = process.env.AMBIGUOUS_API_KEY ?? process.env.FADS_CLAW_TOKEN;
    if (!token)
      throw new AdapterError(
        "Ambiguous.ai API key is not configured.",
        "rejected",
      );
    const resources = fadsResources();
    const locationUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(draft.location)}`;
    const body = {
      title:
      ...
      description: [
        ...
        "",
        `Location: [${draft.location}](${locationUrl})`,
      ...
      ...(process.env.AMBIGUOUS_ASSIGNEE_ID
        ...
        : {}),
      ...(process.env.AMBIGUOUS_PROJECT_ID ?? resources.projectId
        ? { project_id: process.env.AMBIGUOUS_PROJECT_ID ?? resources.projectId }
        : {}),
      ...(process.env.AMBIGUOUS_REPORTED_STATUS_ID ?? resources.statusIds?.reported
        ? { task_status_id: process.env.AMBIGUOUS_REPORTED_STATUS_ID ?? resources.statusIds?.reported }
        : {}),
    };
